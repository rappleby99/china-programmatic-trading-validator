...
class SSEValidator:
    ...
    def validate_broker_code(self, row_num: int, value: str) -> bool:
        ...
        if not re.match(r'^\d{5}$', value):
            self.add_error_for_field(row_num, 1, value, "Must be exactly 5 digits")
            return False

        # Check that broker code matches the FIRM_ID from filename
        if self.firm_id and value != self.firm_id:
            self.add_error_for_field(row_num, 1, value,
    ...
    def validate_client_code(self, row_num: int, value: str) -> bool:
        ...
        if len(value) < 3 or len(value) > 10:
            self.add_error_for_field(row_num, 5, value, "Must be 3-10 characters")
            return False
        return True
    ...
    def validate_date(self, row_num: int, value: str) -> bool:
        ...
        if not re.match(r'^\d{8}$', value):
            self.add_error_for_field(row_num, 8, value, "Must be in YYYYMMDD format")
            return False
        try:
            ...
            if self.submission_date and report_date > self.submission_date:
                self.add_error_for_field(row_num, 8, value,
                              f"Report date cannot be later than submission date ({self.submission_date.strftime('%Y%m%d')})")
                return False
        except ValueError:
            self.add_error_for_field(row_num, 8, value, "Invalid date")
            return False
        return True
    ...
    def validate_leverage_ratio(self, row_num: int, value: str, has_leverage: bool) -> bool:
        ...
        try:
            ratio = float(value)
            if ratio < 100:
                self.add_error_for_field(row_num, 17, value, "Must be >= 100 (leverage ratio cannot be less than 100%)")
                return False
            if not has_leverage and ratio != 100:
                self.add_error_for_field(row_num, 17, value, "Must be 100 when no leverage funds")
                return False
            if has_leverage and ratio <= 100:
                self.add_error_for_field(row_num, 17, value, "Must be > 100 when leverage funds exist")
                return False
        except ValueError:
            self.add_error_for_field(row_num, 17, value, "Must be a valid number")
            return False
        return True
    ...
    def validate_fund_source_ratio(self, row_num: int, ratio_value: str, sources_value: str) -> bool:
        ...
        if not matches:
            self.add_error_for_field(row_num, 13, ratio_value,
    ...
    def validate_leverage_funds(self, row_num: int, leverage_size: str, fund_size: str, has_leverage: bool) -> bool:
        ...
        try:
            ...
            if not has_leverage and lev_val != 0:
                self.add_error_for_field(row_num, 14, leverage_size,
    ...
    def validate_row(self, row_num: int, row_data: list) -> bool:
        ...
        row = {i: (row_data[i].strip() if i < len(row_data) else "") for i in range(42)}

        # Set current row context for error messages (Column C = index 2, Column F = index 5)
        self.current_row_context = {
            "account_name": row.get(2, ""),  # 账户名称 - Column C
            "client_code": row.get(5, "")     # 券商客户编码 - Column F (BCAN)
        }
        ...
        if is_stop:
            required_fields = [0, 1, 2, 5, 8]  # EP name, broker code, account name, client code, date
            for idx in required_fields:
                spec = self.field_specs.get(idx)
        ...
        if not self.validate_broker_code(row_num, row.get(1, "")):
            valid = False

        if not self.validate_client_code(row_num, row.get(5, "")):
            valid = False

        if not self.validate_date(row_num, row.get(8, "")):
            valid = False

        # Numeric validations
        if not self.validate_numeric(row_num, 10, row.get(10, "")):
            valid = False

        if not self.validate_numeric(row_num, 14, row.get(14, "")):
            valid = False

        # Leverage-related validations
        has_leverage = "杠杆资金" in row.get(11, "")

        if not self.validate_leverage_ratio(row_num, row.get(17, ""), has_leverage):
            valid = False

        if not self.validate_leverage_funds(row_num, row.get(14, ""), row.get(10, ""), has_leverage):
            valid = False

        # Fund source ratio validation
        if not self.validate_fund_source_ratio(row_num, row.get(13, ""), row.get(11, "")):
            valid = False
    ...
    def validate_file(self, file_path: str, original_filename: Optional[str] = None) -> tuple[bool, list[ValidationError]]:
        ...
        # Check if Shenzhen exchange (not yet supported)
        if exchange_type == "SHENZHEN":
            self.add_error(
                0, "Exchange", "exchange", 0, exchange_type,
                "Shenzhen Stock Exchange files are not currently supported. "
                "Please use Shanghai format (SH_PGTDRPT_*) or contact support for assistance."
            )
            return False, self.errors
        ...
        for row_num, row_data in enumerate(rows, start=1):
            ...
            if len(row_data) > 5:
                client_code = row_data[5].strip() if row_data[5] else ""
                if client_code:
                    if client_code in client_codes:
                        self.add_error_for_field(row_num, 5, client_code,
            ...
            # Record row result
            account_name = row_data[2].strip() if len(row_data) > 2 else ""
            client_code = row_data[5].strip() if len(row_data) > 5 else ""
    ...
    def _read_xlsx(self, file_path: str) -> list[list[str]]:
        ...
        try:
            ...
            # Find header row (look for "联交所参与者名称")
            ...
            for row_idx in range(header_row + 1, ws.max_row + 1):
                row_data = []
                has_data = False
                for col_idx in range(1, 43):  # 42 columns
